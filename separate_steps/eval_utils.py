...
def compute_results_classifier(behavior_dict, data, cls, cls_params):
    ...
    HF_HARMBENCH_CLASSIFIER_URL = os.getenv("HF_HARMBENCH_CLASSIFIER_URL")
    if HF_HARMBENCH_CLASSIFIER_URL:
        HF_TOKEN = os.getenv("HF_TOKEN")
        openai_client = openai.Client(
            base_url=HF_HARMBENCH_CLASSIFIER_URL,
            api_key=HF_TOKEN
        )
        ...
        for question in inputs:
            try:
                ...
                preds.append(response.choices[0].message.content.strip().lower())
